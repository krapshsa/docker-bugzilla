  ...
  task :migrate_from_bugzilla => :environment do
    ...
    module BugzillaMigrate
      ...
      class BugzillaDescription < ActiveRecord::Base
        self.table_name = :longdescs
        self.inheritance_column = :bongo
      ...
      def self.migrate_users
        ...
        @user_map = {}
        BugzillaProfile.order('userid').all.each do |profile|
      ...
      def self.migrate_products
        ...
        BugzillaProduct.find_each do |product|
          ...
          product.components.each do |component|
            ...
            category.assigned_to = User.find_by_id(uid)
            category.save
      ...
      def self.migrate_issues()
        ...
        BugzillaBug.order("bug_id ASC").all.each  do |bug|
          ...
          issue = Issue.new(
            ...
            :status => STATUS_MAPPING[bug.bug_status] || NEW_STATUS,
          ...
          version = Version.where(:project_id => @project_map[bug.product_id], :name => bug.version).first
          ...
          # Create watchers
          BugzillaCC.where(:bug_id => bug.bug_id).all.each do |cc|
